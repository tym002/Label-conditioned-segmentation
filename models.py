...
from keras.layers import RepeatVector, GlobalAveragePooling3D,Lambda,Input,Reshape, Conv3D,UpSampling3D,Dropout,MaxPooling3D,Concatenate,BatchNormalization,Activation,Conv3DTranspose,LeakyReLU
...
def unet(img_shape, out_ch=1, start_ch=8, depth=3, inc_rate=2., activation='relu',
         dropout=0, batchnorm=False, maxpool=True, upconv=True, residual=False,
         ch_final_layer=8, n_class = 1, label = 'multi'):
    ...
    if label != 'multi':
        o1 = level_block(i1, l1, emb, start_ch, depth, inc_rate, activation, dropout, batchnorm, maxpool, upconv, residual, label)
    else: 
        o1 = level_block(i1, None, None, start_ch, depth, inc_rate, activation, dropout, batchnorm, maxpool, upconv, residual, label)
